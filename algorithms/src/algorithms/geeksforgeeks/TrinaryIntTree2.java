...
public class TrinaryIntTree2 {

	public static void main(String[] args) {

		TrinaryIntTree2 x = new TrinaryIntTree2();
		Tree t = x.new Tree();
		t.insert(5);
		t.insert(10);
		t.insert(2);
	...
	public class Tree {
		...
		/**
		 * Inserts the passed value in the tree at appropriate location
		 * 
		 * @param value
		 */
		...
		private Node insert(int value, Node root) {
			if (value < root.val) {
				...
				if (root.mid != null)
					insert(value, root.mid);
				else
					root.mid = new Node(value);
			}
			return root;
		}
			...
			if (root != null) {
				if (value < root.val) {
					...
					if (root.mid != null) {
						...
						int min = leftMost(root.right).val;
						root.val = min;
		...
		/**
		 * Returns the node with the minimal value in the tree formed by the
		 * passed node.
		 * 
		 * @param root
	...
	/**
	 * Node class represents an individual node in the overall tree. Each node
	 * holds a value, and points to at most 3 more nodes.
	 * 

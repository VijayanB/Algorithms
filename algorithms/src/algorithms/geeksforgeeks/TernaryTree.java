...
public class TernaryTree {
	 ...
	 *
	 * @param newValue
	 *            The integer value to be inserted into the tree.
	 */
	...
	private void insert(Node root, Node newNode) {
		...
		// determine which node the new node should be a child of, insert it in
	...
	/**
	 * Deletes a node from the ternary tree. Searches for the furthest down node
	 * with the chosen value (this only applies to nodes that have children of
	 * equal value). This particular feature could be costly if there are a lot
	 * of repeating values, but does have the advantage of simplifying reference
	 * manipulation. This method also uses a rudimentary form of tree
	 * rebalancing i.e. if a node with multiple children is being deleted, the
	 * left or right child with the most descendants will be rotated upwards.
	 * This will not keep the tree perfectly balanced, but it is better than
	 * nothing. This will run in O(log n) time on average, but will approach
	 * O(n) if there are a lot of repeated values.
	 *
	 * @param value
	...
	private Node delete(Node root, int value) {
		...
		// if the value has been found, delete it. Otherwise, try deleting
		// recursively from the left or right nodes
		if (root.value == value) {
			// if the node has a middle child, delete that instead
			if (root.middle != null) {
				root.middle = delete(root.middle, value);
			} else {
				// if the node is a child of a middle node, it will not have
				// left/right children
